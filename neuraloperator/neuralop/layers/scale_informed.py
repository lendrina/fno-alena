...
#NEW: Embedding scale parameters using power-law features p**(j/(C_emb-1)).
class ParamPowerEmbedding(nn.Module):
    def __init__(self, in_params: int, c_emb: int, out_channels: int):
        super().__init__()
        self.register_buffer("grid", torch.linspace(0, 1, c_emb))
        self.proj = nn.Linear(in_params * c_emb, out_channels, bias=False)
    ...
    def forward(self, params: torch.Tensor) -> torch.Tensor:
        ...
        for s in range(S):
            p = params[:, s:s+1]  # [B,1]
            feats.append(p ** self.grid.to(device=p.device, dtype=p.dtype))  # [B, C_emb]
        H = torch.cat(feats, dim=1)         # [B, S*C_emb]
        G = self.proj(H)                    # [B, out_channels]
        return G
    


class ScaleInformedSpectralConv(BaseSpectralConv):
    """
    Same as SpectralConv, but [CHANGED]:
      - forward takes extra 'params'
      - Fourier coeffs are gated by scalar embeddings before contraction
    """
    ...
    def __init__(
        ...
        init_std="auto",
        fft_norm="forward",
        emb_channels: int = 16,         #NEW: dimension of scalar embedding
        n_params: int = 1,              #NEW: number of scalar params
        device=None,
    ):
        ...
        if factorization is None:
            ...
            self.weight = nn.Parameter(
                init_std * torch.randn(*weight_shape, dtype=torch.cfloat)
            )
        else:
            # factorized version
        ...
        #NEW: parameter embedding module
        self.param_emb = ParamPowerEmbedding(n_params, emb_channels, in_channels)
    ...
    #NEW: forward: extra params
    def forward(
        self,
        x: torch.Tensor,
        params: torch.Tensor,  #NEW: extra scalar params
        output_shape: Optional[Tuple[int]] = None,
    ):
        """
        x:      [B, Cin, d1, ..., dN]
        params: [B, S] scalar parameters (time, A,B,...)
        """
        ...
        # SAME: slicing logic for selecting modes
